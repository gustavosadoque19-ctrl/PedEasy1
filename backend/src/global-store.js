...
class QueryBuilder {
  ...
  _execute() {
    const db = getDb();
    try {
      if (this.insertData) return { data: this._doInsert(db), error: null };
      if (this.updateData) return { data: this._doUpdate(db), error: null };
      if (this.deleteMode) return { data: null, error: this._doDelete(db) };
      return this._doSelect(db);
    } finally {
      db.close();
    }
  }
  ...
  _doSelect(db) {
    const { clause, params } = this._where();
    ...
    if (this.headMode) {
      const row = db.prepare(`SELECT COUNT(*) as count FROM ${this.table} ${clause}`).get(...params);
  ...
  _doInsert(db) {
    ...
    for (const item of data) {
      ...
      const result = db.prepare(`INSERT INTO ${this.table} (${cols.join(', ')}) VALUES (${ph})`).run(...vals);

      if (this.singleMode || this.selected !== '*') {
        const row = db.prepare(`SELECT * FROM ${this.table} WHERE id = ?`).get(result.lastInsertRowid);
  ...
  _doUpdate(db) {
    ...
    db.prepare(`UPDATE ${this.table} SET ${setClause} ${clause}`).run(...vals, ...params);
