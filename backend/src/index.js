...
function crudRoutes(collection) {
  ...
  router.get('/:id', authMiddleware, (req, res) => {
    const item = getById(collection, Number(req.params.id));
  ...
  router.put('/:id', authMiddleware, (req, res) => {
    const item = update(collection, Number(req.params.id), req.body);
    if (!item) return res.status(404).json({ error: `${collection} não encontrado` });
    res.json(item);
  });

  router.delete('/:id', authMiddleware, (req, res) => {
    const ok = remove(collection, Number(req.params.id));
...
app.get('/api/cardapio/produtos', (req, res) => {
  const produtos = getAll('produtos').filter((p) => p.ativo !== false);
  const adicionais = getAll('adicionais').filter((a) => a.ativo !== false);
  const categorias = getAll('categorias-adicionais').filter((c) => c.ativo !== false);
...
pedidosRouter.get('/', authMiddleware, (req, res) => {
  let data = getAll('pedidos');
...
pedidosRouter.get('/abertos', authMiddleware, (req, res) => {
  const data = query('pedidos', (p) => p.status !== 'fechado' && p.status !== 'cancelado');
  res.json(data);
});

pedidosRouter.get('/:id', authMiddleware, (req, res) => {
  const item = getById('pedidos', Number(req.params.id));
  if (!item) return res.status(404).json({ error: 'Pedido não encontrado' });
  res.json(item);
});

pedidosRouter.post('/', authMiddleware, (req, res) => {
  const item = create('pedidos', req.body);
  res.status(201).json(item);
});

pedidosRouter.put('/:id', authMiddleware, (req, res) => {
  const item = getById('pedidos', Number(req.params.id));
  if (!item) return res.status(404).json({ error: 'Pedido não encontrado' });

  const updated = update('pedidos', Number(req.params.id), req.body);

  const newStatus = req.body.status;
  const oldStatus = item.status;
  if ((newStatus === 'entregue' || newStatus === 'fechado') && oldStatus !== newStatus) {
    const caixas = query('caixa', (c) => c.status === 'aberto');
    if (caixas.length > 0) {
      const caixa = caixas[0];
      const mov = {
        id: (caixa.movimentos?.length || 0) + 1,
        caixa_id: caixa.id,
        tipo: 'entrada',
...
pedidosRouter.delete('/:id', authMiddleware, (req, res) => {
  const ok = remove('pedidos', Number(req.params.id));
...
app.get('/api/estoque', authMiddleware, (req, res) => {
  res.json(getAll('estoque'));
});

app.post('/api/estoque', authMiddleware, (req, res) => {
  const item = create('estoque', req.body);
  res.status(201).json(item);
});

app.get('/api/caixa/aberto', authMiddleware, (req, res) => {
  const caixas = query('caixa', (c) => c.status === 'aberto');
  res.json(caixas[0] || null);
});

app.post('/api/caixa/abrir', authMiddleware, (req, res) => {
  const item = create('caixa', { ...req.body, status: 'aberto', movimentos: [] });
  res.status(201).json(item);
});

app.post('/api/caixa/fechar/:id', authMiddleware, (req, res) => {
  const caixa = getById('caixa', Number(req.params.id));
  ...
  const updated = update('caixa', caixa.id, {
    status: 'fechado',
...
app.post('/api/caixa/movimento', authMiddleware, (req, res) => {
  const caixa = getById('caixa', Number(req.body.caixa_id));
  if (!caixa) return res.status(404).json({ error: 'Caixa não encontrado' });
  const mov = { ...req.body, id: (caixa.movimentos?.length || 0) + 1, createdAt: new Date().toISOString() };
  const updated = update('caixa', caixa.id, {
    movimentos: [...(caixa.movimentos || []), mov],
  });
...
app.get('/api/caixa/historico', authMiddleware, (req, res) => {
  res.json(getAll('caixa'));
});

app.post('/api/cardapio/pedidos', (req, res) => {
...
app.get('/api/config/delivery', (req, res) => {
  const stored = getAll('config_delivery');
...
app.put('/api/config/delivery', (req, res) => {
  const stored = getAll('config_delivery');
  const body = req.body;
  if (body.horarios) {
  ...
  if (stored.length > 0) {
    const updated = update('config_delivery', stored[0].id, body);
    res.json(updated);
  } else {
    const item = create('config_delivery', body);
    res.json(item);
  }
});

app.get('/api/relatorios/vendas', authMiddleware, (req, res) => {
  const pedidos = getAll('pedidos').filter((p) => p.status === 'fechado' || p.status === 'entregue');
...
app.get('/api/fidelidade/config', authMiddleware, (req, res) => {
  const configs = getAll('fidelidade_config');
  res.json(configs[0] || { pontos_por_real: 1, pontos_minimo_resgate: 100, valor_resgate_por_ponto: 0.05, ativo: true });
});

app.put('/api/fidelidade/config', authMiddleware, (req, res) => {
  const configs = getAll('fidelidade_config');
  if (configs.length > 0) {
    const updated = update('fidelidade_config', configs[0].id, req.body);
    res.json(updated);
  } else {
    const item = create('fidelidade_config', req.body);
    res.json(item);
  }
});

app.get('/api/nps/resumo', authMiddleware, (req, res) => {
  const pesquisas = getAll('nps');
...
app.post('/api/fidelidade/pontos', authMiddleware, (req, res) => {
  const { cliente_id, pontos } = req.body;
  let clientes = getAll('fidelidade_clientes');
  let record = clientes.find((c) => c.cliente_id === cliente_id);
  if (record) {
    record = update('fidelidade_clientes', record.id, { pontos: (record.pontos || 0) + pontos });
  } else {
    record = create('fidelidade_clientes', { cliente_id, pontos, total_gasto: 0 });
  }
  res.json(record);
});

app.post('/api/fidelidade/resgatar', authMiddleware, (req, res) => {
  const { cliente_id, pontos } = req.body;
  const clientes = getAll('fidelidade_clientes');
  ...
  const updated = update('fidelidade_clientes', record.id, { pontos: (record.pontos || 0) - pontos });
  res.json(updated);
});

app.post('/api/carrinhos/:id/lembrete', authMiddleware, (req, res) => {
  const carrinho = getById('carrinhos', Number(req.params.id));
  if (!carrinho) return res.status(404).json({ error: 'Carrinho não encontrado' });
  const updated = update('carrinhos', carrinho.id, { whatsapp_enviado: true });
  res.json({ success: true, message: 'Lembrete enviado' });
});

app.post('/api/pagamentos/pix', authMiddleware, (req, res) => {
  const { pedido_id, valor } = req.body;
  const item = create('pagamentos', { pedido_id, valor, forma: 'pix', status: 'pendente', qr_code: `pix-qr-${Date.now()}`, cobranca: `cob-${Date.now()}` });
  res.status(201).json(item);
});

app.post('/api/pagamentos/cartao', authMiddleware, (req, res) => {
  const { pedido_id, valor, parcelas } = req.body;
  const forma = parcelas > 1 ? 'cartao_credito' : 'cartao_debito';
  const item = create('pagamentos', { pedido_id, valor, forma, status: 'aprovado', transacao_id: `txn-${Date.now()}` });
  res.status(201).json(item);
});

app.post('/api/integracoes/:id/sync', authMiddleware, (req, res) => {
  const integracao = getById('integracoes', Number(req.params.id));
  if (!integracao) return res.status(404).json({ error: 'Integração não encontrada' });
  const updated = update('integracoes', integracao.id, { ultima_sync: new Date().toISOString() });
  res.json({ success: true, message: 'Sincronizado com sucesso' });
});

app.get('/api/relatorios', authMiddleware, (req, res) => {
  res.json(getAll('relatorios'));
});
...
app.post('/api/produtos/:id/imagem', authMiddleware, (req, res) => {
  upload.single('imagem')(req, res, (err) => {
    ...
    const produto = getById('produtos', Number(req.params.id));
    if (!produto) return res.status(404).json({ error: 'Produto não encontrado' });
    const updated = update('produtos', produto.id, { imagem: req.file.filename });
    res.json(updated);
  });
});
