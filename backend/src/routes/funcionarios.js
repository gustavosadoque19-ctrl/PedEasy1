...
router.get('/', authMiddleware, async (req, res) => {
  const data = await getAll('funcionarios');
  const semSenha = data.map(({ senha, ...rest }) => rest);
  res.json(semSenha);
});

router.get('/:id', authMiddleware, async (req, res) => {
  const item = await getById('funcionarios', Number(req.params.id));
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  const existentes = await getAll('funcionarios');
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  const item = await update('funcionarios', Number(req.params.id), payload);
...
router.delete('/:id', authMiddleware, async (req, res) => {
  const ok = await remove('funcionarios', Number(req.params.id));
